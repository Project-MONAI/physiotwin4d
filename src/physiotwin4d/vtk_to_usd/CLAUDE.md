...
top-level PhysioTwin4D modules. The in-repository entry point for VTK-to-USD
conversion is:

```python
from physiotwin4d.convert_vtk_to_usd import ConvertVTKToUSD
```

External advanced users may import `physiotwin4d.vtk_to_usd` directly.
...
PhysioTwin4D keeps images and surfaces in ITK's native LPS frame; the
