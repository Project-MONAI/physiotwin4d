"""Base class for PhysioTwin4D providing standardized logging and debug settings.

This module provides the PhysioTwin4DBase class that can be inherited by other
classes in the PhysioTwin4D package to provide consistent logging and messaging
functionality instead of scattered print statements.

All classes share a common logger called "PhysioTwin4D" but include their class
...
Example:
    ...
    >>> import logging
    >>>
    >>> class MyClass(PhysioTwin4DBase):
    ...     def __init__(self):
    ...
    >>> PhysioTwin4DBase.set_log_classes(['MyClass', 'OtherClass'])
    >>>
...
class PhysioTwin4DBase:
    """Base class providing standardized logging and debug settings.

    This class provides a consistent logging interface for all PhysioTwin4D
    classes. All classes share a common logger called "PhysioTwin4D" but include
    their class name in log messages for identification.

    Class Attributes:
        _shared_logger (logging.Logger): Shared logger for all PhysioTwin4D classes
    ...
    Example:
        >>> import logging
        >>> class MyRegistration(PhysioTwin4DBase):
        ...     def __init__(self):
        ...
        >>>
        >>> # Filter to show only specific classes
        >>> PhysioTwin4DBase.set_log_classes(['MyRegistration'])
        >>>
        >>> # Show all classes again
        >>> PhysioTwin4DBase.set_log_all_classes()
    """
    ...
    _class_filter: Optional[ClassNameFilter] = None
    ...
    def __init__(
        ...
        if not PhysioTwin4DBase._logger_initialized:
            PhysioTwin4DBase._initialize_shared_logger(log_level, log_to_file)

        # Get reference to shared logger
        assert PhysioTwin4DBase._shared_logger is not None
        self.logger: logging.Logger = PhysioTwin4DBase._shared_logger
    ...
    def _initialize_shared_logger(
        cls, log_level: int | str, log_to_file: Optional[str] = None
    ) -> None:
        ...
        cls._shared_logger = logging.getLogger("PhysioTwin4D")
    ...
    def log_section(
        self, title: str, *args: Any, width: int = 70, char: str = "="
    ) -> None:
