...
for patient_dir in patient_dirs:
    ...
    if recompute:
        ...
        for time_id in time_series_ids:
            image_path = patient_output_dir / f"{patient_id}_g{time_id}_warped_ref.mha"
    ...
    for image_index, image in enumerate(reconstructed_images):
        ...
        if recompute:
            ...
            itk.transformwrite(
                inv_tfm,
                str(patient_output_dir / f"{patient_id}_g{time_id}_inverse_tfm.hdf"),
            )
            ...
            itk.imwrite(
                labelmap,
                str(
                    patient_output_dir / f"{patient_id}_g{time_id}_ref_labelmap.nii.gz"
                ),
                compression=True,
            )
        else:
        ...
        surface_path = patient_output_dir / f"{patient_id}_g{time_id}_ssm_surface.vtp"
